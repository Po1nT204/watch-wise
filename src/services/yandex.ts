...
ИНСТРУКЦИИ:
1. SUMMARY: Напиши четкий образовательный конспект в формате Markdown. Используй заголовки (##), жирный текст для терминов и маркированные списки. Убери "мусор", мелкие ошибки транскрипта, приветствия и повторы.
2. QUESTIONS: Сгенерируй ровно ${settings.count} вопросов. 
   - Сложность "${settings.difficulty}": 
     * 'easy' — вопросы на знание конкретных фактов;
     * 'medium' — вопросы на понимание логики и причин;
     * 'hard' — вопросы на анализ и выводы из сказанного.
   - ВАЖНО ПО ТАЙМКОДАМ: Поле "timestamp" должно СТРОГО равняться числу в скобках [Xs] из текста. Выбирай метку, которая идет СРАЗУ ПОСЛЕ того, как мысль была полностью закончена, чтобы видео остановилось в логической паузе.
