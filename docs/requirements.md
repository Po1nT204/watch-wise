...
- **VK Video:**
  1.  Загрузка аудиодорожки на сервер с помощью библиотек (`yt-dlp` wrapper / `play-dl` / `ytdlp-nodejs`).
  2.  Отправка аудиофайла в Yandex SpeechKit (ASR API) для получения транскрипта с временными метками.
- **YouTube:**
  1.  Получение субтитров через специализированные парсеры (`youtube-captions-scraper` или другие библиотеки).
